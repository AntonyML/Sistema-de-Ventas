...
{
    public partial class Consulta_linea : Form
    {
        public Consulta_linea()
        {
            InitializeComponent();
        }
        SqlConnection conexion = new SqlConnection("server=ROBINSON\\ROBINSON;database=servicio_de_ventas;integrated security=true");
        private void Consulta_linea_Load(object sender, EventArgs e)
        {
        ...
        {
            conexion.Open();
            string consulta = "SELECT * FROM linea WHERE LineaNo = '" + txtFiltro.Text + "'";
            SqlDataAdapter adapt = new SqlDataAdapter(consulta, conexion);
            DataTable dt = new DataTable();
            adapt.Fill(dt);
