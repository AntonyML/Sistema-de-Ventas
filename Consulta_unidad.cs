...
{
    public partial class Consulta_unidad : Form
    {
        public Consulta_unidad()
        {
            InitializeComponent();
        }
        SqlConnection conexion = new SqlConnection("server=ROBINSON\\ROBINSON;database=servicio_de_ventas;integrated security=true");
      
        private void Consulta_unidad_Load(object sender, EventArgs e)
        {
        ...
        {
            conexion.Open();
            string consulta = "SELECT * FROM unidad WHERE unidadNo = '" + txtFiltro.Text + "'";
            SqlDataAdapter adapt = new SqlDataAdapter(consulta, conexion);
            DataTable dt = new DataTable();
            adapt.Fill(dt);
